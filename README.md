...
ES modules require a static server (not `file://`). Serve the **workspace
root** — the About Me portrait is the canonical shared asset under
`templates/research-dashboard/assets/author/`:

```sh
python -m http.server 8137     # from the workspace root
# open http://localhost:8137/ai-job-postings/dashboard/
node verify.mjs                # dataset + template self-check (run in this folder)
```
